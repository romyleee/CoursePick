...
def _match_rule(ans: Answers) -> dict:
    for rule in _data()["rules"]:
        ...
        if m["state"] == ans.state and m["mood"] == ans.mood and m["activity"] == ans.activity:
            return rule
    return {"options": [{"course_types": ["cafe", "food", "dessert"], "reason": "기본 추천 코스"}]}
...
def _build_one_option(
    ...
    """course_types 시퀀스로 코스 1개 생성. cross_exclude로 다른 옵션과 중복 방지."""
    course: list[CourseStep] = []
    picked_full: list[dict] = []
    used: set[str] = set()
    for idx, ctype in enumerate(course_types, start=1):
    ...
    if not course:
        return None
    reason = f"{extra_reason}\n{base_reason}".strip() if extra_reason else base_reason
...
def build_course(
    ...
    """N개의 서로 다른 코스 옵션 생성. 각 옵션은 룰의 다른 vibe 시퀀스 사용."""
    rule = _match_rule(answers)
    rule_options = rule.get("options") or [{"course_types": rule.get("course_types", []), "reason": rule.get("reason", "")}]
    ...
    for i in range(n):
        opt_def = rule_options[i % len(rule_options)]
        opt = _build_one_option(
            answers,
            opt_def["course_types"],
            opt_def["reason"],
            exclude_recent,
            cross_exclude,
            favorites,
